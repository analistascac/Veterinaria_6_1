...
public class frmNuevaCompra extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1793687930831374669L;
	private JPanel contentPane;
	private JTextField txtCantidad;
	private JList<String> lstProductos;
	private DefaultListModel<String> canasto = new DefaultListModel<String>();
	...
	public frmNuevaCompra() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				frmNuevaCompra.class.getResource("/Images/logo.jpg")));
		...
		cmbProveedor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (cn.conectarDB()) {
					este = (Proveedor) cn.devolverProveedores().get(
							cmbProveedor.getSelectedIndex());
		...
		cmbTipoFactura.setModel(new DefaultComboBoxModel(new String[] { "A",
				"B", "C" }));
		...
		txtCantidad.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent arg0) {
				...
				if (Auxiliar.isInteger(txtCantidad.getText())
					...
					if (camposOK()) {
						btnAgregarAlCanasto.setEnabled(true);
					}
				} else {
					btnAgregarAlCanasto.setEnabled(false);
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!txtPrecioCompra.getText().trim().isEmpty()
					...
					if (cn.conectarDB()) {
						...
						p = (Proveedor) cn.devolverProveedores().get(
								cmbProveedor.getSelectedIndex());
						e = (Empleado) cn.devolverEmpleados().get(
								cmbEmpleado.getSelectedIndex());
						prod = (Producto) cn.devolverProveedorProductos(p).get(
								cmbProducto.getSelectedIndex());

						try {
							com.setCantidad(Integer.parseInt(txtCantidad
									.getText()));
							com.setPrecio_costo(Double
									.parseDouble(txtPrecioCompra.getText()));
							com.setPrecio_venta(Double
									.parseDouble(txtPrecioVenta.getText()));
							if (prod.getNombre() != null) {
								canasto.addElement(prod.getNombre() + " x "
										+ txtCantidad.getText());
							} else {
								canasto.addElement(prod.getNombreCientifico()
										+ " x " + txtCantidad.getText());
							}
							com.setIdProveedor(p.getId());
							com.setIdProducto(prod.getId());
							com.setIdEmpleado(e.getId());
							com.setTipo_factura(cmbTipoFactura
									.getSelectedItem().toString());
							com.setEstadoOperacion(cmbEstadoOperacion
									.getSelectedItem() + "");
							txtCantidad.setText("");
							if (!error)
								compra.add(com);
		...
		txtPrecioVenta.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent arg0) {
				...
				if (Auxiliar.isDouble(txtPrecioVenta.getText())
						&& Double.parseDouble(txtPrecioVenta.getText()) > 0) {
					...
					btnAgregarAlCanasto.setEnabled(false);
					txtPrecioVenta.setForeground(Color.red);
				}
		...
		lblNewLabel.setIcon(new ImageIcon(frmNuevaCompra.class
				.getResource("/Images/image_marca_agua.png")));
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int x = JOptionPane.showConfirmDialog(null,
						"¿Confirma dar de alta la nueva compra?",
				...
				if (x == JOptionPane.YES_OPTION) {
					...
					if (cn.conectarDB()) {
						try {
							...
							JOptionPane.showMessageDialog(null,
									"Compra cargada exitosamente",
									"Información",
									JOptionPane.INFORMATION_MESSAGE);
							dispose();
						} catch (Exception e) {
							JOptionPane.showMessageDialog(null,
									"Error en la alta de la compra.", "Error",
									JOptionPane.ERROR_MESSAGE);
						}
					} else {
						JOptionPane.showMessageDialog(null,
		...
		if (cn.conectarDB()) {
			...
			for (int i = 0; i < prodooctos.size(); i++) {
				if (prodooctos.get(i).getNombre() != null) {
					...
					productos.addElement(prodooctos.get(i)
							.getNombreCientifico());
				}
	...
	private void llenarEmpleados() {
		...
		if (cn.conectarDB()) {
			...
			for (int i = 0; i < empleaados.size(); i++) {
				empleados.addElement(empleaados.get(i).getNombre() + " "
						+ empleaados.get(i).getApellido() + " ~ "
						+ empleaados.get(i).getId());
			}
		} else {
			JOptionPane.showMessageDialog(null,
					"Error en la conexion de base de datos", "Error",
	...
	private boolean camposOK() {
		...
		return txtCantidad.getText().length() > 0
